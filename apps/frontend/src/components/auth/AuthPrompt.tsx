  ...
  return (
    <div className="relative rounded-3xl border border-slate-200 bg-white p-5 shadow-lg">
      ...
      <div className="mt-4 grid gap-3">
        {Object.entries(providerDetails).map(([provider, meta]) => (
          <Button
            key={provider}
            type="button"
            full
          ...
          >
            ...
            <span className="flex-1 text-center">Continue with {meta.label}</span>
          </Button>
        ))}
      </div>
      ...
      <div className="mt-5 rounded-2xl border border-slate-200 bg-slate-50 p-4">
        ...
        <form
          ...
          onSubmit={view === "signin" ? handleCredentialsSignIn : handleRegister}
        >
          <div>
            <Label>Email</Label>
          ...
          {!guestMode ? (
            ...
            <form className="grid gap-3" onSubmit={handleGuestSubmit}>
              ...
              <p className="text-xs text-slate-500">
                We’ll only use this name for this single {context === "book" ? "book entry" : "comment"}. You can create an
