...
function PersonIcon() {
  return (
    <svg aria-hidden viewBox="0 0 24 24" className="h-6 w-6 text-slate-500" fill="currentColor">
    ...
    <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/80 backdrop-blur">
      <div className="mx-auto flex w-full max-w-6xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
        ...
        <nav className="flex items-center gap-4 text-sm text-slate-600">
          <Link href="/books/new" className="hidden rounded-full border border-violet-200 px-4 py-2 font-medium text-violet-700 transition hover:bg-violet-50 sm:inline-flex">
            Add book
          </Link>
          <button
            type="button"
          ...
          >
            {isAuthenticated ? <UserAvatar name={displayName} /> : <PersonIcon />}
